...
const Box = ({ qoutes, onclick }) => {
  return (
    <div className="p-6 relative  justify-center mt-60 w-1/4 bg-darkGrayishBlue mx-auto flex flex-col items-center rounded-xl  md:w-2/6">
      <div>
        <h3 className="text-neonGreen text-center tracking-wide mb-3">
          A D V I C E #{qoutes?.id}
        </h3>
        <p className="text-center text-2xl text-bold text-lightCyan md:text-">
          "{qoutes?.advice}"
        </p>
      </div>
      <img src={Pattern} alt="pattern" className="mt-2 hidden md:flex " />
      <img src={PatternMobile} alt="pattern " className="mt-5 flex md:hidden" />
